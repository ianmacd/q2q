...
static char const *tas256x_rcv_text[] = {"DISABLE", "ENABLE", "ENABLE_32OHM"};
static const struct soc_enum tas256x_rcv_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(tas256x_rcv_text),
		tas256x_rcv_text),
};
...
static const struct snd_kcontrol_new tas256x_left_controls[] = {
	...
	SOC_SINGLE_EXT("TAS256X CLASSH TIMER LEFT", CLASSH_TIMER, 1, 22, 0,
		tas256x_get, tas256x_put),
	SOC_ENUM_EXT("TAS256X RECEIVER ENABLE LEFT", tas256x_rcv_enum[0],
		tas256x_enum_get, tas256x_enum_put),
};
...
static const struct snd_kcontrol_new tas256x_right_controls[] = {
	...
	SOC_SINGLE_EXT("TAS256X CLASSH TIMER RIGHT", CLASSH_TIMER, 2, 22, 0,
		tas256x_get, tas256x_put),
	SOC_ENUM_EXT("TAS256X RECEIVER ENABLE RIGHT", tas256x_rcv_enum[0],
		tas256x_enum_get, tas256x_enum_put),
};
...
static irqreturn_t tas256x_irq_thread(int irq, void *dev_id)
{
...
/* Added for device re-init and sysfs node support */
struct tas256x_sysfs *g_tas_sysfs;
static ssize_t tas256x_init_show(struct device *dev,
...
static ssize_t tas256x_init_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t size)
{
	...
	if (IS_ERR(g_tas_sysfs))  {
		pr_err("tas256x %s failed !! memory not allocated yet!!\n",
			__func__);
		ret = PTR_ERR(g_tas_sysfs);
		goto end;
	}

	ret = kstrtos32(buf, 10, &value);
	if (ret) {
		pr_err("tas256x:%s Invalid input\n", __func__);
		goto end;
	}
	if (g_tas_sysfs && g_tas_sysfs->p_tas256x)
		tas256x_reinit(g_tas_sysfs->p_tas256x);
end:
	return size;
}

static DEVICE_ATTR(reinit, 0664, tas256x_init_show,
			tas256x_init_store);

static struct attribute *tas256x_attr[] = {
	&dev_attr_reinit.attr,
	NULL,
};

static struct attribute_group tas256x_attr_grp = {
		.attrs = tas256x_attr,
};

void tas256x_delete_sysfs_node(struct tas256x_priv *p_tas256x)
{
	struct tas256x_sysfs *tas_sysfs = NULL;

	if (p_tas256x == NULL) {
		pr_err("%s:p_tas256x is NULL\n", __func__);
		return;
	}
	tas_sysfs = p_tas256x->tas_sysfs;
	if (tas_sysfs) {
		if (tas_sysfs->reinit_dev) {
			sysfs_remove_group(&tas_sysfs->reinit_dev->kobj,
				&tas256x_attr_grp);
			device_destroy(tas_sysfs->tas256x_class, 1);
		}

		if (tas_sysfs->tas256x_class)
			class_destroy(tas_sysfs->tas256x_class);
	}
	g_tas_sysfs = NULL;
	kfree(p_tas256x->tas_sysfs);
}

int tas256x_add_sysfs_node(struct tas256x_priv *p_tas256x)
{
	int32_t ret = 0;
	struct tas256x_sysfs *tas_sysfs = NULL;
	...
	p_tas256x->tas_sysfs = kzalloc(sizeof(struct tas256x_sysfs), GFP_KERNEL);

	if (!p_tas256x->tas_sysfs)
		return ret;

	tas_sysfs = p_tas256x->tas_sysfs;
	tas_sysfs->p_tas256x = p_tas256x;
	g_tas_sysfs = tas_sysfs;

	tas_sysfs->tas256x_class = class_create(THIS_MODULE, TAS256X_SYSFS_CLASS_NAME);
	if (IS_ERR(tas_sysfs->tas256x_class)) {
		ret = PTR_ERR(tas_sysfs->tas256x_class);
		pr_err("%s err tas256x_class create\n", __func__);
		tas_sysfs->tas256x_class = NULL;
		goto err_dev;
	}

	tas_sysfs->reinit_dev = device_create(tas_sysfs->tas256x_class,
				NULL, 1, NULL, TAS256X_CODEC_DIR_NAME);
	if (IS_ERR(tas_sysfs->reinit_dev)) {
		pr_err("%s Failed to create reinit_dev\n", __func__);
		ret = PTR_ERR(tas_sysfs->reinit_dev);
		tas_sysfs->reinit_dev = NULL;
		goto err_dev;
	}

	ret = sysfs_create_group(&tas_sysfs->reinit_dev->kobj,
		&tas256x_attr_grp);
	if (ret) {
...
err_dev:
	tas256x_delete_sysfs_node(p_tas256x);
	pr_err("%s Error %d\n", __func__, ret);
	return ret;
}
					...
					struct tas256x_priv *p_tas256x)
{
	...
	rc = of_property_read_u32(np, "ti,rcv-only", &p_tas256x->rcv_only);
	if (rc) {
		p_tas256x->rcv_only = 0;
		dev_dbg(plat_data->dev,
				"Looking up %s property in node %s failed %d\n",
			"ti,rcv-only", np->full_name, rc);
	} else {
		dev_dbg(plat_data->dev, "ti,rcv-only=0x%x",
			p_tas256x->rcv_only);
	}
			...
			const struct i2c_device_id *id)
{
...
#endif
	/* mn_power_state is under p_tas256x structure */
	...
	for (i = 0; i < p_tas256x->mn_channels; i++) {
		if (gpio_is_valid(p_tas256x->devs[i]->mn_irq_gpio)) {
			...
			/* Modified to add irq thread to use IRQF_TRIGGER_LOW instead
			 * instead of IRQF_TRIGGER_FALLING
			 */
	...
	/* Default value should be false to avoid abnormal IRQs
	 * should only be enabled during playback.
	 */
...
#if IS_ENABLED(CONFIG_TAS256X_REINIT)
	tas256x_add_sysfs_node(p_tas256x);
#endif
